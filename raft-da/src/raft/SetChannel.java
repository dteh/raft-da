...
/**
 * 
 * @author danielteh FindNodes() Uses broadcast to find the addresses of all
 *         other nodes who have joined the 'channel'. Saves list of IPs to a
 *         list. Rescans for new addresses once every second
 *
...
public class SetChannel extends ReceiverAdapter implements Runnable {
	...
	/**
	 * Gets the IPs available in the cluster (Is this necessary? Perhaps replace
	 * with node ID finding)
	 * 
	 * @throws Exception
	 */
	private void getAddresses() throws Exception {
		while (true) {
			...
			/*
			 * This converts member IDs in cluster to IP addresses Probably
			 * don't need this... for (org.jgroups.Address member: members){
			 * PhysicalAddress physicalAddr = (PhysicalAddress)channel.down(new
			 * Event(Event.GET_PHYSICAL_ADDRESS,member)); IpAddress ipAddr =
			 * (IpAddress)physicalAddr;
